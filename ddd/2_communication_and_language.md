...
### 글로 쓴 설계 문서
1. 문서는 코드와 말을 보완하는 역할
    - XP (eXtreme Programming) 에서는..
      - 문서를 전혀 사용하지 않고, 코드 스스로 별도의 설명이 필요없는 상태를 유지해야 한다
      - 프로그램의 실제 동작 + 실행 가능한 테스트에만 집중
    - 하지만 설계 문서로써의 코드는 한계가 있음
    - 문서는 **의미를 설명, 대규모 구조를 서술, 핵심 요소**에 집중해야 함
2. 문서는 유효한 상태를 유지하고 최신 내용을 담고 있어야 한다
    - 문서의 가장 큰 가치: 모델의 개념을 설명, 코드의 세부사항을 파악해 나가는 데 도움, 모델의 의도된 사용 방식에 대한 설명
    - **문서는 프로젝트 활동과 관련을 맺고 있어야 한다** - UBIQUITOUS LANGUAGE 와 상호작용 하는지 확인
    - 문서에 설명된 용어가 대화와 코드에 나타나지 않는다면, 문서가 본연의 목적을 수행하지 못하고 있음을 의미
    - or 문서가 뒤처지는 과정에서 UBIQUITOUS LANGUAGE 가 자연스럽게 변화하는 것을 파악할 수도
    - 문서를 최소한으로 유지하고 코드와 대화를 보완하는데 집중 -> 문서를 포르젝트와 연관된 상태로 유지할 수 있음

### 실행 가능한 기반 (Executable Bedrock)
3장의 MODEL-DRIVEN DESIGN 을 통해 의미 전달이 가능한 코드를 작성하는 방법을 볼 예정

올바르게 실행 + 올바른 의미를 전달하는 코드를 작성

불일치를 제거하는 것은 10장 선언적 설계와 같은 접근법에서 강조 (declarative?)

## 설명을 위한 모델 (Explanatory Models)
#### 모델은
- 구현, 설계, 의사소통의 기초가 되어야 한다
- 도메인의 일반 지식을 전달하기 위해 교육적인 도구로 사용되어 도움을 줄 수 있음
  - 그림이나 단어 등의 다른 형태로도 모델을 전달할 수도 있음

<ins>개발 프로세스를 이끄는 기술적 모델</ins>은 해당 기능을 수행하는 데 필요한 **최소한의 수준으로 범위를 줄여야 한다**  
<ins>설명을 위한 모델</ins>은 도메인의 여러 측면들을 포함할 수 있음 -> 좁은 범위의 모델을 명확하게 하는 맥락을 제공

UML 보단 바로 파악이 가능한 다른 형태가 더 좋음